...
short *dataPop()
{
  ...
  pRet = configuration.pDataRoot->pBuffer;
...
int main(int argc, char *argv[]) 
{
  ...
  if(isDetached()) {
    ...
    if(pID == 0) { //child
      ...
      DEBUG("Waiting for process thread...");
...
void *processThreadFunction(void *threadid)
{
  ...
  while(1) {
    //acquire data:
    //we could also use a pThread condition here, to avoid polling, grabbing CPU
    while(buffer == NULL) {
      pthread_mutex_lock(&configuration.dataMutex);
      buffer = dataPop();
      pthread_mutex_unlock(&configuration.dataMutex);
    ...
    //encode
    write = lame_encode_buffer_interleaved(lame, buffer, configuration.bufferSize, mp3_buffer, MP3_BUFFER_SIZE);
    //write mp3 to disk                                                                                             
